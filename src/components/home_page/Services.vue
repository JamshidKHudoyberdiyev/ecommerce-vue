<template>
  <div class="servicese bg-grey-lighten-2 py-10">
    <v-container fluid>
      <v-row class="align-center justify-space-around">
        <v-col cols="2" v-for="(ser, i) in Services">
          <v-hover v-slot="{ isHovering, props }">
            ...
            >
              <div class="parent">
                <span v-html="ser.icon"></span>

                <v-card-title
                  style="font-weight: 700; text-transform: uppercase"
                >
                  {{ ser.title }}</v-card-title
                >
                <v-card-text> {{ ser.text }}</v-card-text>
              </div>
            </v-card>
          </v-hover>
        </v-col>
